...
#define BPAWN   BLACK+PAWN                                                 //BPAWN   EQU     BLACK+PAWN
                                                                           //
//                                                                         //;***********************************************************
// 1) TABLES                                                               //; TABLES SECTION
//                                                                         //;***********************************************************
//***********************************************************              //START:
// TABLES SECTION                                                          //        ORG     START+80H
//***********************************************************              //
//                                                                         //
...
//                                                                         //;***********************************************************
// 2) PTRS to TABLES                                                       //        ORG     START+2FFH
//                                                                         //STACK:
                                                                           //
//***********************************************************              //;***********************************************************
// 2) TABLE INDICES SECTION                                                //; TABLE INDICES SECTION
//                                                                         //;
// M1-M4   --  Working indices used to index into                          //; M1-M4   --  Working indices used to index into
//             the board array.                                            //;             the board array.
//                                                                         //;
// T1-T3   --  Working indices used to index into Direction                //; T1-T3   --  Working indices used to index into Direction
//             Count, Direction Value, and Piece Value tables.             //;             Count, Direction Value, and Piece Value tables.
//                                                                         //;
// INDX1   --  General working indices. Used for various                   //; INDX1   --  General working indices. Used for various
// INDX2       purposes.                                                   //; INDX2       purposes.
//                                                                         //;
// NPINS   --  Number of Pins. Count and pointer into the                  //; NPINS   --  Number of Pins. Count and pointer into the
//             pinned piece list.                                          //;             pinned piece list.
//                                                                         //;
// MLPTRI  --  Pointer into the ply table which tells                      //; MLPTRI  --  Pointer into the ply table which tells
//             which pair of pointers are in current use.                  //;             which pair of pointers are in current use.
//                                                                         //;
// MLPTRJ  --  Pointer into the move list to the move that is              //; MLPTRJ  --  Pointer into the move list to the move that is
//             currently being processed.                                  //;             currently being processed.
//                                                                         //;
// SCRIX   --  Score Index. Pointer to the score table for                 //; SCRIX   --  Score Index. Pointer to the score table for
//             the ply being examined.                                     //;             the ply being examined.
//                                                                         //;
// BESTM   --  Pointer into the move list for the move that                //; BESTM   --  Pointer into the move list for the move that
//             is currently considered the best by the                     //;             is currently considered the best by the
//             Alpha-Beta pruning process.                                 //;             Alpha-Beta pruning process.
//                                                                         //;
// MLLST   --  Pointer to the previous move placed in the move             //; MLLST   --  Pointer to the previous move placed in the move
//             list. Used during generation of the move list.              //;             list. Used during generation of the move list.
//                                                                         //;
...
//                                                                         //;
// 4) MOVE ARRAY                                                           //; MLEND   --  The address of the last available location
//                                                                         //;             in the move list.
                                                                           //;
//***********************************************************              //; MLPTR   --  The Move List is a linked list of individual
// MOVE LIST SECTION                                                       //;             moves each of which is 6 bytes in length. The
//                                                                         //;             move list pointer(MLPTR) is the link field
// MLIST   --  A 2048 byte storage area for generated moves.               //;             within a move.
//             This area must be large enough to hold all                  //;
//             the moves for a single leg of the move tree.                //; MLFRP   --  The field in the move entry which gives the
//                                                                         //;             board position from which the piece is moving.
// MLEND   --  The address of the last available location                  //;
//             in the move list.                                           //; MLTOP   --  The field in the move entry which gives the
//                                                                         //;             board position to which the piece is moving.
// MLPTR   --  The Move List is a linked list of individual                //;
//             moves each of which is 6 bytes in length. The               //; MLFLG   --  A field in the move entry which contains flag
//             move list pointer(MLPTR) is the link field                  //;             information. The meaning of each bit is as
//             within a move.                                              //;             follows:
//                                                                         //;             Bit 7  --  The color of any captured piece
// MLFRP   --  The field in the move entry which gives the                 //;                        0 -- White
//             board position from which the piece is moving.              //;                        1 -- Black
//                                                                         //;             Bit 6  --  Double move flag (set for castling and
// MLTOP   --  The field in the move entry which gives the                 //;                        en passant pawn captures)
//             board position to which the piece is moving.                //;             Bit 5  --  Pawn Promotion flag; set when pawn
//                                                                         //;                        promotes.
// MLFLG   --  A field in the move entry which contains flag               //;             Bit 4  --  When set, this flag indicates that
//             information. The meaning of each bit is as                  //;                        this is the first move for the
//             follows:                                                    //;                        piece on the move.
//             Bit 7  --  The val(COLOR) of any captured piece             //;             Bit 3  --  This flag is set is there is a piece
//                        0 -- White                                       //;                        captured, and that piece has moved at
//                        1 -- Black                                       //;                        least once.
//             Bit 6  --  Double move flag (set for castling and           //;             Bits 2-0   Describe the captured piece.  A
//                        en passant pawn captures)                        //;                        zero value indicates no capture.
//             Bit 5  --  Pawn Promotion flag; set when pawn               //;
//                        promotes.                                        //; MLVAL   --  The field in the move entry which contains the
//             Bit 4  --  When set, this flag indicates that               //;             score assigned to the move.
//                        this is the first move for the                   //;
//                        piece on the move.                               //;***********************************************************
//             Bit 3  --  This flag is set is there is a piece             //        ORG     START+300H
...
MP20:   LD      (a,b);                  //  Counter for direction          //MP20:   LD      a,b             ; Counter for direction
        CP      (3);                    //  On diagonal moves ?            //        CP      3               ; On diagonal moves ?
        JR      (C,MP35);               //  Yes - Jump                     //        JR      C,MP35          ; Yes - Jump
        JR      (Z,MP30);               //  -or-jump if on 2 square move   //        JR      Z,MP30          ; -or-jump if on 2 square move
        Z80_EXAF;                       //  Is forward square empty?       //        EX      af,af'          ; Is forward square empty?
        JR      (NZ,MP15);              //  No - jump                      //        JR      NZ,MP15         ; No - jump
        LD      (a,val(M2));            //  Get "to" position              //        LD      a,(M2)          ; Get "to" position
        CP      (91);                   //  Promote white Pawn ?           //        CP      91              ; Promote white Pawn ?
        JR      (NC,MP25);              //  Yes - Jump                     //        JR      NC,MP25         ; Yes - Jump
        CP      (29);                   //  Promote black Pawn ?           //        CP      29              ; Promote black Pawn ?
        JR      (NC,MP26);              //  No - Jump                      //        JR      NC,MP26         ; No - Jump
MP25:   LD      (hl,addr(P2));          //  Flag address                   //MP25:   LD      hl,P2           ; Flag address
        SET     (5,ptr(hl));            //  Set promote flag               //        SET     5,(hl)          ; Set promote flag
MP26:   CALLu   (ADMOVE);               //  Add to move list               //MP26:   CALL    ADMOVE          ; Add to move list
        INC     (iy);                   //  Adjust to two square move      //        INC     iy              ; Adjust to two square move
        DEC     (b);                    //                                 //        DEC     b
        LD      (hl,addr(P1));          //  Check Pawn moved flag          //        LD      hl,P1           ; Check Pawn moved flag
        BIT     (3,ptr(hl));            //  Has it moved before ?          //        BIT     3,(hl)          ; Has it moved before ?
        JR      (Z,MP10);               //  No - Jump                      //        JR      Z,MP10          ; No - Jump
        JPu     (MP15);                 //  Jump                           //        JP      MP15            ; Jump
MP30:   Z80_EXAF;                       //  Is forward square empty ?      //MP30:   EX      af,af'          ; Is forward square empty ?
        JR      (NZ,MP15);              //  No - Jump                      //        JR      NZ,MP15         ; No - Jump
MP31:   CALLu   (ADMOVE);               //  Add to move list               //MP31:   CALL    ADMOVE          ; Add to move list
        JPu     (MP15);                 //  Jump                           //        JP      MP15            ; Jump
MP35:   Z80_EXAF;                       //  Is diagonal square empty ?     //MP35:   EX      af,af'          ; Is diagonal square empty ?
        JR      (Z,MP36);               //  Yes - Jump                     //        JR      Z,MP36          ; Yes - Jump
        LD      (a,val(M2));            //  Get "to" position              //        LD      a,(M2)          ; Get "to" position
        CP      (91);                   //  Promote white Pawn ?           //        CP      91              ; Promote white Pawn ?
        JR      (NC,MP37);              //  Yes - Jump                     //        JR      NC,MP37         ; Yes - Jump
        CP      (29);                   //  Black Pawn promotion ?         //        CP      29              ; Black Pawn promotion ?
        JR      (NC,MP31);              //  No- Jump                       //        JR      NC,MP31         ; No- Jump
MP37:   LD      (hl,addr(P2));          //  Get flag address               //MP37:   LD      hl,P2           ; Get flag address
        SET     (5,ptr(hl));            //  Set promote flag               //        SET     5,(hl)          ; Set promote flag
        JRu     (MP31);                 //  Jump                           //        JR      MP31            ; Jump
MP36:   CALLu   (ENPSNT);               //  Try en passant capture         //MP36:   CALL    ENPSNT          ; Try en passant capture
        JPu     (MP15);                 //  Jump                           //        JP      MP15            ; Jump
}                                                                          //
                                                                           //
                                                                           //
...
// for dummy move                                                          //                                ; for dummy move
        LD      (val(M1),a);            //                                 //        LD      (M1),a
        LD      (val(M2),a);            //                                 //        LD      (M2),a
        LD      (a,val(P3));            //  Save captured Pawn             //        LD      a,(P3)          ; Save captured Pawn
        LD      (val(P2),a);            //                                 //        LD      (P2),a
        CALLu   (ADMOVE);               //  Add Pawn capture to move list  //        CALL    ADMOVE          ; Add Pawn capture to move list
        LD      (a,val(M3));            //  Restore "from" position        //        LD      a,(M3)          ; Restore "from" position
        LD      (val(M1),a);            //                                 //        LD      (M1),a
}                                                                          //
                                                                           //
                                                                           //
...
AT14B:  BIT     (5,d);                  //  Opposite val(COLOR) found already?//AT14B:  BIT     5,d             ; Opposite color found already?
        ...
        SET     (6,d);                  //  Set same val(COLOR) found flag //        SET     6,d             ; Set same color found flag
                                                                           //
//                                                                         //;
// ***** DETERMINE IF PIECE ENCOUNTERED ATTACKS SQUARE *****               //; ***** DETERMINE IF PIECE ENCOUNTERED ATTACKS SQUARE *****
AT14:   LD      (a,val(T2));            //  Fetch piece type encountered   //AT14:   LD      a,(T2)          ; Fetch piece type encountered
        LD      (e,a);                  //  Save                           //        LD      e,a             ; Save
        LD      (a,b);                  //  Get direction-counter          //        LD      a,b             ; Get direction-counter
        CP      (9);                    //  Look for Knights ?             //        CP      9               ; Look for Knights ?
        JR      (C,AT25);               //  Yes - jump                     //        JR      C,AT25          ; Yes - jump
        LD      (a,e);                  //  Get piece type                 //        LD      a,e             ; Get piece type
        CP      (QUEEN);                //  Is is a Queen ?                //        CP      QUEEN           ; Is is a Queen ?
        JR      (NZ,AT15);              //  No - Jump                      //        JR      NZ,AT15         ; No - Jump
        SET     (7,d);                  //  Set Queen found flag           //        SET     7,d             ; Set Queen found flag
        JRu     (AT30);                 //  Jump                           //        JR      AT30            ; Jump
AT15:   LD      (a,d);                  //  Get flag/scan count            //AT15:   LD      a,d             ; Get flag/scan count
        AND     (0x0F);                 //  Isolate count                  //        AND     0FH             ; Isolate count
        CP      (1);                    //  On first position ?            //        CP      1               ; On first position ?
        JR      (NZ,AT16);              //  No - jump                      //        JR      NZ,AT16         ; No - jump
        LD      (a,e);                  //  Get encountered piece type     //        LD      a,e             ; Get encountered piece type
        CP      (KING);                 //  Is it a King ?                 //        CP      KING            ; Is it a King ?
        JR      (Z,AT30);               //  Yes - jump                     //        JR      Z,AT30          ; Yes - jump
AT16:   LD      (a,b);                  //  Get direction counter          //AT16:   LD      a,b             ; Get direction counter
        CP      (13);                   //  Scanning files or ranks ?      //        CP      13              ; Scanning files or ranks ?
        JR      (C,AT21);               //  Yes - jump                     //        JR      C,AT21          ; Yes - jump
        LD      (a,e);                  //  Get piece type                 //        LD      a,e             ; Get piece type
        CP      (BISHOP);               //  Is it a Bishop ?               //        CP      BISHOP          ; Is it a Bishop ?
        JR      (Z,AT30);               //  Yes - jump                     //        JR      Z,AT30          ; Yes - jump
        LD      (a,d);                  //  Get flags/scan count           //        LD      a,d             ; Get flags/scan count
        AND     (0x0F);                 //  Isolate count                  //        AND     0FH             ; Isolate count
        CP      (1);                    //  On first position ?            //        CP      1               ; On first position ?
        JR      (NZ,AT12);              //  No - jump                      //        JR      NZ,AT12         ; No - jump
        CP      (e);                    //  Is it a Pawn ?                 //        CP      e               ; Is it a Pawn ?
        JR      (NZ,AT12);              //  No - jump                      //        JR      NZ,AT12         ; No - jump
        LD      (a,val(P2));            //  Fetch piece including val(COLOR)//        LD      a,(P2)          ; Fetch piece including color
        BIT     (7,a);                  //  Is it white ?                  //        BIT     7,a             ; Is it white ?
        JR      (Z,AT20);               //  Yes - jump                     //        JR      Z,AT20          ; Yes - jump
        LD      (a,b);                  //  Get direction counter          //        LD      a,b             ; Get direction counter
        CP      (15);                   //  On a non-attacking diagonal ?  //        CP      15              ; On a non-attacking diagonal ?
        JR      (C,AT12);               //  Yes - jump                     //        JR      C,AT12          ; Yes - jump
        JRu     (AT30);                 //  Jump                           //        JR      AT30            ; Jump
AT20:   LD      (a,b);                  //  Get direction counter          //AT20:   LD      a,b             ; Get direction counter
        CP      (15);                   //  On a non-attacking diagonal ?  //        CP      15              ; On a non-attacking diagonal ?
        JR      (NC,AT12);              //  Yes - jump                     //        JR      NC,AT12         ; Yes - jump
        JRu     (AT30);                 //  Jump                           //        JR      AT30            ; Jump
AT21:   LD      (a,e);                  //  Get piece type                 //AT21:   LD      a,e             ; Get piece type
        CP      (ROOK);                 //  Is is a Rook ?                 //        CP      ROOK            ; Is is a Rook ?
        JR      (NZ,AT12);              //  No - jump                      //        JR      NZ,AT12         ; No - jump
        JRu     (AT30);                 //  Jump                           //        JR      AT30            ; Jump
AT25:   LD      (a,e);                  //  Get piece type                 //AT25:   LD      a,e             ; Get piece type
        CP      (KNIGHT);               //  Is it a Knight ?               //        CP      KNIGHT          ; Is it a Knight ?
        JR      (NZ,AT12);              //  No - jump                      //        JR      NZ,AT12         ; No - jump
AT30:   LD      (a,val(T1));            //  Attacked piece type/flag       //AT30:   LD      a,(T1)          ; Attacked piece type/flag
        CP      (7);                    //  Call from POINTS ?             //        CP      7               ; Call from POINTS ?
        JR      (Z,AT31);               //  Yes - jump                     //        JR      Z,AT31          ; Yes - jump
        BIT     (5,d);                  //  Is attacker opposite val(COLOR) ?//        BIT     5,d             ; Is attacker opposite color ?
        JR      (Z,AT32);               //  No - jump                      //        JR      Z,AT32          ; No - jump
        LD      (a,1);                  //  Set attacker found flag        //        LD      a,1             ; Set attacker found flag
        JPu     (AT13);                 //  Jump                           //        JP      AT13            ; Jump
AT31:   CALLu   (ATKSAV);               //  Save attacker in attack list   //AT31:   CALL    ATKSAV          ; Save attacker in attack list
AT32:   LD      (a,val(T2));            //  Attacking piece type           //AT32:   LD      a,(T2)          ; Attacking piece type
        CP      (KING);                 //  Is it a King,?                 //        CP      KING            ; Is it a King,?
        JP      (Z,AT12);               //  Yes - jump                     //        JP      Z,AT12          ; Yes - jump
        CP      (KNIGHT);               //  Is it a Knight ?               //        CP      KNIGHT          ; Is it a Knight ?
        JP      (Z,AT12);               //  Yes - jump                     //        JP      Z,AT12          ; Yes - jump
        JPu     (AT10);                 //  Jump                           //        JP      AT10            ; Jump
 }                                                                         //
                                                                           //
//***********************************************************              //
// ATTACK SAVE ROUTINE                                                     //;***********************************************************
...
// return (P-K4)                                                           //                                ; return (P-K4)
        CP      (35);                   //  Is it a King Pawn ?            //        CP      35              ; Is it a King Pawn ?
        RET     (Z);                    //  Yes - return (P-K4)            //        RET     Z               ; Yes - return (P-K4)
BM9:    INC     (ptr(hl));              //  (P-Q4)                         //BM9:    INC     (hl)            ; (P-Q4)
        INC     (ptr(hl));              //                                 //        INC     (hl)
        INC     (ptr(hl));              //                                 //        INC     (hl)
        RETu;                           //  Return to CPTRMV               //        RET                     ; Return to CPTRMV
}                                                                          //
                                                                           //;*******************************************************
//                                                                         //; GRAPHICS DATA BASE
//  Omit some Z80 User Interface code, eg                                  //;*******************************************************
//  user interface data including graphics data and text messages          //; DESCRIPTION:  The Graphics Data Base contains the
//  Also macros                                                            //;               necessary stored data to produce the piece
//      CARRET, CLRSCR, PRTLIN, PRTBLK and EXIT                            //;               on the board. Only the center 4 x 4 blocks are
//  and functions                                                          //;               stored and only for a Black Piece on a White
//      DRIVER and INTERR                                                  //;               square. A White piece on a black square is
...
// and return info about it                                                //                                ; and return info about it
        LD      (a,b);                  //  Special move flags             //        LD      a,b             ; Special move flags
        AND     (a);                    //  Special ?                      //        AND     a               ; Special ?
        JR      (NZ,CP10);              //  Yes - jump                     //        JR      NZ,CP10         ; Yes - jump
        LD      (d,e);                  //  "To" position of the move      //        LD      d,e             ; "To" position of the move
        CALLu   (BITASN);               //  Convert to Ascii               //        CALL    BITASN          ; Convert to Ascii
        LD      (val16(MVEMSG+3),hl);   //  Put in move message            //        LD      (MVEMSG+3),hl   ; Put in move message
        LD      (d,c);                  //  "From" position of the move    //        LD      d,c             ; "From" position of the move
        CALLu   (BITASN);               //  Convert to Ascii               //        CALL    BITASN          ; Convert to Ascii
        LD      (val16(MVEMSG),hl);     //  Put in move message            //        LD      (MVEMSG),hl     ; Put in move message
        //PRTBLK  (MVEMSG,5);             //  Output text of move          //        PRTBLK  MVEMSG,5        ; Output text of move
        JRu     (CP1C);                 //  Jump                           //        JR      CP1C            ; Jump
CP10:   BIT     (1,b);                  //  King side castle ?             //CP10:   BIT     1,b             ; King side castle ?
        JR      (Z,rel020);             //  No - jump                      //        JR      Z,rel020        ; No - jump
        //PRTBLK  (addr(O_O),5);                //  Output "O-O"           //        PRTBLK  O_O,5           ; Output "O-O"
        JRu     (CP1C);                 //  Jump                           //        JR      CP1C            ; Jump
rel020: BIT     (2,b);                  //  Queen side castle ?            //rel020: BIT     2,b             ; Queen side castle ?
        JR      (Z,rel021);             //  No - jump                      //        JR      Z,rel021        ; No - jump
        //PRTBLK  (addr(O_O_O),5);              //  Output "O-O-O"         //        PRTBLK  O_O_O,5         ; Output "O-O-O"
        JRu     (CP1C);                 //  Jump                           //        JR      CP1C            ; Jump
rel021: //PRTBLK  (P_PEP,5);              //  Output "PxPep" - En passant  //rel021: PRTBLK  P_PEP,5         ; Output "PxPep" - En passant
CP1C:   LD      (a,val(COLOR));         //  Should computer call check ?   //CP1C:   LD      a,(COLOR)       ; Should computer call check ?
        LD      (b,a);                                                     //        LD      b,a
        XOR     (0x80);                 //  Toggle val(COLOR)              //        XOR     80H             ; Toggle color
        LD      (val(COLOR),a);                                            //        LD      (COLOR),a
        CALLu   (INCHK);                //  Check for check                //        CALL    INCHK           ; Check for check
        AND     (a);                    //  Is enemy in check ?            //        AND     a               ; Is enemy in check ?
        LD      (a,b);                  //  Restore val(COLOR)             //        LD      a,b             ; Restore color
        LD      (val(COLOR),a);                                            //        LD      (COLOR),a
        JR      (Z,CP24);               //  No - return                    //        JR      Z,CP24          ; No - return
        CARRET();                       //  New line                       //        CARRET                  ; New line
        LD      (a,val(SCORE+1));       //  Check for player mated         //        LD      a,(SCORE+1)     ; Check for player mated
        CP      (0xFF);                 //  Forced mate ?                  //        CP      0FFH            ; Forced mate ?
        CALL    (NZ,TBCPMV);            //  No - Tab to computer column    //        CALL    NZ,TBCPMV       ; No - Tab to computer column
        //PRTBLK  (CKMSG,5);              //  Output "check"               //        PRTBLK  CKMSG,5         ; Output "check"
        LD      (hl,addr(LINECT));      //  Address of screen line count   //        LD      hl,LINECT       ; Address of screen line count
        INC     (ptr(hl));              //  Increment for message          //        INC     (hl)            ; Increment for message
CP24:   LD      (a,val(SCORE+1));       //  Check again for mates          //CP24:   LD      a,(SCORE+1)     ; Check again for mates
        CP      (0xFF);                 //  Player mated ?                 //        CP      0FFH            ; Player mated ?
        RET     (NZ);                   //  No - return                    //        RET     NZ              ; No - return
        LD      (c,0);                  //  Set player mate flag           //        LD      c,0             ; Set player mate flag
        CALLu   (FCDMAT);               //  Full checkmate ?               //        CALL    FCDMAT          ; Full checkmate ?
        RETu;                           //  Return                         //        RET                     ; Return
}                                                                          //
                                                                           //;***********************************************************
...
//  FCDMAT, TBPLCL, TBCPCL, TBPLMV and TBCPMV                              //; FUNCTION:   --  To examine situations where there exits
...
//  PLYRMV                                                                 //; FUNCTION:   --  To accept and validate the players move
...
//  Omit some more Z80 user interface stuff, functions                     //;***********************************************************
//  CHARTR, PGIFND, MATED, ANALYS                                          //; FUNCTION:   --  Accepts a single character input from the
...
//                                                                         //; SET UP EMPTY BOARD
//  Omit some more Z80 user interface stuff, functions                     //;***********************************************************
//  DSPBRD, BSETUP, INSPCE, CONVRT                                         //; FUNCTION:   --  Display graphics board and pieces.
//                                                                         //;
                                                                           //; CALLED BY:  --  DRIVER
//***********************************************************              //
